...
function Wechat(config, callback) {
  ...
  return function*() {
    ...
    if (!this.query.code) {
      ...
      state = state || '';
      var url = `https://open.weixin.qq.com/connect/qrconnect?appid=${this.config.appID}&redirect_uri=http%3A%2F%2F${this.config.domain}${encodeURIComponent(this.config.redirect)}&response_type=code&scope=snsapi_login&state=${state}`;
      this.redirect(url);
      return;
    }
