...
function Wechat(config, callback) {
  ...
  return function*() {
    ...
    if (!this.query.code) {
      ...
      var url = `https://open.weixin.qq.com/connect/qrconnect?appid=${this.config.appID}&redirect_uri=http%3A%2F%2F${this.config.domain}${encodeURIComponent(this.path)}&response_type=code&scope=snsapi_login&state=login`;
      this.redirect(url);
      return;
    }
    ...
    yield callback.call(this, this.userInfo, this.token);
  };
